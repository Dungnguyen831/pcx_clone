<div class="card" style="background: white; padding: 30px; border-radius: 10px; border-top: 5px solid #27ae60; box-shadow: 0 4px 6px rgba(0,0,0,0.1); max-width: 600px; margin: 0 auto;">
    ...
    <div class="card" style="margin-top: 20px; background: #f4f7f6; padding: 30px; border-radius: 10px; border-top: 5px solid #2980b9; box-shadow: 0 4px 6px rgba(0,0,0,0.1); max-width: 600px; margin: 20px auto;">
        <h3 style="color: #2980b9; margin-bottom: 20px;">
            <i class="fa-solid fa-file-excel"></i> Nhập Kho Bằng File Excel
        </h3>
        
        <form action="index.php?controller=warehouse&action=processExcelImport" method="POST" enctype="multipart/form-data">
            <div class="form-group" style="margin-bottom: 20px;">
                <label style="display:block; font-weight:bold; margin-bottom: 8px;">Chọn file Excel (.xlsx):</label>
                <input type="file" name="excel_file" accept=".xlsx" required style="width:100%; padding:10px; border:1px solid #ddd; background: white;">
            </div>

            <button type="submit" style="width:100%; padding:15px; background:#2980b9; color:white; border:none; border-radius:5px; cursor:pointer; font-weight:bold;">
                <i class="fa-solid fa-upload"></i> TẢI LÊN VÀ XỬ LÝ
            </button>
        </form>
    </div>
</div>