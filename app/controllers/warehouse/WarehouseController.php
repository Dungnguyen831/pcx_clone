...
class WarehouseController {
    ...
    public function processImport() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $price      = (float)$_POST['price']; // Giá nhập mới
            $note       = $_POST['note'] ?? 'Nhập hàng mới';
            $user_id    = $_SESSION['user_id'] ?? 1; 

            try {
                // 1. Tạo phiếu nhập (Bảng imports)
                // Lưu ý: Sửa lại cú pháp nối chuỗi cho an toàn hơn chút
                ...
                // 3. Cập nhật số lượng tồn kho (Bảng inventory)
                $this->db->execute("UPDATE inventory SET quantity = quantity + $quantity WHERE product_id = $product_id");

                // =================================================================================
                // 4. MỚI: CẬP NHẬT GIÁ NHẬP CHO SẢN PHẨM (Bảng products)
                // Logic: Lấy giá $price vừa nhập ở trên -> Gán thẳng vào import_price của sản phẩm
                // =================================================================================
                $this->db->execute("UPDATE products SET import_price = $price WHERE product_id = $product_id");
